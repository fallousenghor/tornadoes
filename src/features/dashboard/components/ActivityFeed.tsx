        ...
        {activitiesData.map((a, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center',
            ...
            <div style={{ width: 32, height: 32, borderRadius: '50%', flexShrink: 0, marginRight: 10,
              background: `${a.avatarColor}22`, border: `1px solid ${a.avatarColor}44`,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 10, fontWeight: 700, color: a.avatarColor, fontFamily: "'DM Sans',sans-serif" }}>
              {a.avatarInitials}
            </div>
            <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 9, color: Colors.textDim, fontFamily: "'DM Sans',sans-serif", minWidth: 50, textAlign: 'right' }}>
              {Math.floor((Date.now() - a.timestamp.getTime()) / 60000) < 60 
                ? `${Math.floor((Date.now() - a.timestamp.getTime()) / 60000)} min` 
                : `${Math.floor((Date.now() - a.timestamp.getTime()) / 3600000)}h`} ago
            </div>
          </div>
        ))}
      </div>
