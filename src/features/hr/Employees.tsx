...
export const Employees: React.FC = () => {
  ...
  const handleExportEmployees = () => {
    const exportData = filteredEmployees.map(emp => ({
      ...
      'Téléphone': emp.phone || '',
      'Poste': emp.poste,
      'Département': emp.departmentName,
      'Statut': emp.status,
      'Type Contrat': emp.contractType,
      'Salaire': emp.salary,
      'Date Début': emp.startDate,
      'Notes': emp.notes || '',
    }));
    exportToCSV(exportData, 'employes');
    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <Button variant="secondary" onClick={handleExportEmployees}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {paginatedEmployees.map((emp, index) => {
                const statusStyle = getStatusColor(emp.status, colors);
                const contractStyle = getContractTypeColor(emp.contractType, colors);
                return (
                  <tr 
                    key={emp.id} 
                    style={{ 
                      borderBottom: `1px solid ${colors.border}`,
                      background: index % 2 === 0 ? 'transparent' : colors.hover,
                    }}
                  >
