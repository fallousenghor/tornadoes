...
export const Students: React.FC = () => {
  ...
  const handleStudentSubmit = async (data: StudentFormData) => {
    try {
      if (data.id) {
        ...
        toast.success('Apprenant mis à jour', 'Les modifications ont été enregistrées');
      } else {
        // Create new student
        ...
        toast.success('Apprenant créé', 'Le nouvel apprenant a été ajouté avec succès');
      }
      setIsFormOpen(false);
  ...
  const handleDeleteStudent = async (student: StudentDisplay) => {
    if (!window.confirm(`Êtes-vous sûr de vouloir supprimer l'apprenant ${student.firstName} ${student.lastName} ?`)) {
      return;
    }
    try {
      await studentService.deleteStudent(student.id);
      toast.success('Apprenant supprimé', `${student.firstName} ${student.lastName} a été supprimé`);
      fetchStudents();
    } catch (err) {
      console.error('Error deleting student:', err);
      toast.error('Erreur de suppression', 'Impossible de supprimer l\'apprenant');
    }
  };
  ...
  const handleExportStudents = () => {
    const exportData = students.map(student => ({
      ...
      'Date Naissance': student.birthDate || '',
      'Statut': student.status,
      'Programme': student.programName || '',
      'Date Inscription': student.enrollmentDate || '',
    }));
    exportToCSV(exportData, 'etudiants');
    toast.success('Export réussi', 'Les données ont été exportées en CSV');
  };
                        ...
                        >
                          ...
                          <td style={{ padding: '14px 16px', textAlign: 'center' }}>
                            <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
                              ...
                              <button
                                style={{
                                  padding: '6px 12px', borderRadius: 6, border: `1px solid rgba(224, 80, 80, 0.3)`,
                                  background: 'transparent', color: '#e05050', fontSize: 11, cursor: 'pointer',
                                }}
